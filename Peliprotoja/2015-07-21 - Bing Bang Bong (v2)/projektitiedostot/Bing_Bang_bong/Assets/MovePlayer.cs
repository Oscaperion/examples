...
public class MovePlayer : MonoBehaviour {
	...
	void Ammutaanko() {
		if (Input.GetButtonDown ("Fire1") && ammustenMaara < maxAmmusMaara) { 
			...
			newAmmo.AddComponent <PlayerAmmoBehav> ();
			ammustenMaara++;
			GetComponent<AudioSource>().Play();
			//newAmmo.GetComponent<PlayerAmmoBehav> ().AnnaLentorata (Camera.main.ScreenToWorldPoint (Input.mousePosition), transform.position);
		} 
	}
	...
	public bool Tuhotaanko() {
		...
		VoittoEsitys.GetComponent<LopputulosIlmoitus> ().EsitaVihollisVoitto ();
		TuhoaOmatAmmukset ();
		return true;
	}

	void TuhoaOmatAmmukset() {
		GameObject[] ammukset = GameObject.FindGameObjectsWithTag ("PlayerDisks");
	...
	void siirraHahmoa(int siirtoluku) {
		...
		if (vali > kerto) {
			...
			TuhoaOmatAmmukset ();
			VoittoEsitys.GetComponent<LopputulosIlmoitus>().EsitaPutousHavio();
			Destroy(gameObject);
			//suorita = false;
		} else {
